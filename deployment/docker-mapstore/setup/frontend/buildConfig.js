...
module.exports = (bundles, themeEntries, paths, extractThemesPlugin, prod, publicPath, cssPrefix, prodPlugins, alias = {}, proxy) => ({
    ...
    // Use local backend.
    // To use online backend, comment this devServer, and uncomment devServer section at the end
    devServer: {
        proxy: {
            '/rest/': {
		// We can set the env var when building image.
		// Uncomment this when we finally know how to
    ...
    // Uncomment to use online Mapstore backend
    // devServer: {
    //     proxy: proxy || {
    //         '/rest/geostore': {
